...
model = BinaryModel(freeze_backbone=False).to(device)
...
def train_loop():
    for epoch in range(EPOCHS):
        ...
        for images, labels in train_loader:
            ...
            train_acc += BinaryModel.binary_acc(logits.detach(), labels)
...
def test_loop():
    ...
    with torch.inference_mode():
        for images, labels in test_loader:
            ...
            test_acc += BinaryModel.binary_acc(logits, labels)
            test_metrics = BinaryModel.binary_metrics(logits, labels)
